...
class Player():
    def __init__(self, population, index=0):
        ...
        self.index = index
        self.round_history = []
        log.debug('Created player object with population '+str(self.company)+' under index '+str(self.index))
...
class Game():
    ...
    def player_generator(self, input_population = [[3,0],[1,2],[2,1]]):
        ...
        for i in range(1, len(input_population)):
            if len(input_population[i-1])!=len(input_population[i]):
                error_text = 'Inconsistent population sizes for indexes '+str(i-1)+' and '+str(i)
    ...
    def get_state(self):
        self.state = np.array([[c.company[0] for c in self.players],
                               [c.company[1] for c in self.players]])
        return self
    ...
    def is_strategy_nash(self, strategy):
        ...
        '''
        is_nash = True
        base_payoff = self.payoff[''.join(map(str, strategy))]#np.sum(self.state, axis=0)
...
def simulate(strategy = [1,1,0,0,0], start_population = [[5, 10], [5, 5], [15, 5], [15, 5], [30, 5]]):
    game = Game()
    population = []
